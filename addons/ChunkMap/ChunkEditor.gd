...
#			floor(mpos.x / (CHUNK_SIZE.x * cell_size.x)),
#			floor(mpos.y / (CHUNK_SIZE.y * cell_size.y))
#		)
		if chunk_pos != _active_chunk_pos:
			if _active_chunk_pos in _chunks:
				clear()
				_chunks[_active_chunk_pos].modulate = Color(1,1,1,0.5)
